...
# 来跟踪计算是哪些数据通过哪些操作组合起来产生输出。 自动微分使系统能够随后反向传播梯度。
...
# 默认值是None
x.grad  
# 打印结果：tensor(28., grad_fn=<MulBackward0>)
y = 2 * torch.dot(x, x)

# x是一个长度为4的向量，计算x和x的点积，得到了我们赋值给y的标量输出。
# 接下来，通过调用反向传播函数来自动计算y关于x每个分量的梯度，并打印这些梯度。
y.backward()
# 打印结果：tensor([ 0.,  4.,  8., 12.])
x.grad
# 梯度应为4 * x，打印结果应该为tensor([True, True, True, True])
x.grad == 4 * x
...
z = u * x
z.sum().backward()
# 打印结果：tensor([True, True, True, True])
x.grad == u
